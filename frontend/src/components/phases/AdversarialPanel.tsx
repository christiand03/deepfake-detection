...
import type { AnalysisResult, Phase4Result, AttentionShift } from '../../types/analysis'

interface AdversarialPanelProps {
  result: AnalysisResult | null
}

// ── Attention shift table ────────────────────────────────────────────────────

function AttentionShiftTable({ shifts }: { shifts: AttentionShift[] }) {
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {shifts.map(s => {
          ...
          return (
            <div key={s.region} style={{ display: 'grid', gridTemplateColumns: '72px 1fr 1fr 52px', gap: 6, alignItems: 'center' }}>
              {/* Region label */}
              <span
                style={{
                  fontSize: 10,

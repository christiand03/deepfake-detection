...
> Abstract-grounded draft. Wording follows the **allowed wording** in [[Claim Map]]; hedged claims are marked. `needs-full-text` numbers are deliberately omitted. [[istvt-2023|ISTVT]] is cited qualitatively only (metadata-only). Language: English (translate to German if the Belegarbeit is written in German). Replace inline `(Author Year)` with the final citation style before submission.

...
| `analyze_chunk_distribution.py` | 128 | Gegenstück zu `analyze_metadata.py` auf **Chunk-Ebene**: liest `data/processed/*_metadata.csv` und gibt Umfang je Split, Fake-Rate je Label-Spalte sowie die Gruppierung `split × modify_type × label*` aus — die Ebene, auf der tatsächlich trainiert wird. Ergebnisse in `datasets.md` §7. | **[E]** |

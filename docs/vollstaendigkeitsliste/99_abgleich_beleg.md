...
> - **Die Statusspalte der neuen Zeilen ist überwiegend `✗`.** Die Kapitelskizzen stammen
>   vom 2026-08-01, die Implementierung ist zweieinhalb Wochen jünger — sie kann dort
>   nicht vermerkt sein. Das ist kein Versäumnis, aber es heißt: **diese Zeilen sind
>   vollständig ungeschrieben** und ohne dieses Register nicht auffindbar.
...
| D52 | **`loc_lambda: 0` ist ein echter Kontrollarm, kein Ausschalter.** Bei λ = 0 wird die Relevanz mit `create_graph=False` berechnet und geloggt, erreicht die Gewichte aber nicht; der Lauf hat damit die Trajektorie eines gewöhnlichen CE-Finetunings **und** eine Lokalisierungsspur. `tests/test_attnlrp_patch_scope.py` weist beide Richtungen nach (`create_graph=False` erreicht die Gewichte nicht, `True` schon) | 02 / 09 | 05, 06 | ✗ | **Neu angelegt 2026-08-21** · Ohne diese Zeile wirkt der Kontrolllauf wie ein weggelassenes Experiment statt wie ein konstruierter Vergleich. **Er trägt das Hauptergebnis** (siehe F25d). Umfang: ein Absatz |
...
| F25d | **Das Ergebnis der Relevanz-Regularisierung**, 911 Chunks aus 624 Test-Clips, alle Arme schrittgleich bei Batch 6.000: Baseline 1,921 · **Kontrolle λ=0 → 1,867** · Aux-Head 2,200 · λ=0,02 → 8,210 · λ=0,1 → 11,418 (`ratio_over_chance`), bei `val/auc_video` 1,0000 / 1,0000 / 0,9953 / 0,9854 / 0,9444. Beide λ>0-Arme haben Konfidenzintervalle, die die Kontrolle nicht überlappen | 04 / 12 | **06** | ✗ | **Neu angelegt 2026-08-21** · **Der wichtigste Einzelwert ist 1,867**: Die Kontrolle liegt mit demselben Trainingsbudget, aber ohne Strafterm, *nicht über* der Baseline. Weitertrainieren allein bringt keinen Lokalisierungsgewinn — der gesamte Effekt ist dem Strafterm zuzurechnen, ohne Abschlag für „hat halt länger trainiert". Genau diese Frage stellt ein Betreuer zuerst, und sie ist beantwortet. Rohwerte zitierfähig aus `docs/results/` (nicht aus dem Vault, dort fehlen sie). Umfang: Tabelle + zwei Absätze |
| F25e | **Drei Vorbehalte, die mit F25d zusammen genannt werden müssen.** (1) **Nicht auskonvergiert:** `docs/results/training_curve.csv` zeigt, dass die Lokalisierung bei Batch 6.000 noch *beschleunigt* (λ=0,02: +0,774 → +1,800 je 1.000 Batches), während die Kontrolle exakt flach bleibt (−0,000/1k) — die Zahlen sind **untere Schranken** eines abgeschnittenen Laufs. (2) **Einzelläufe**, `seed=42`, keine Seed-Varianz. (3) **Die Läufe trainieren ohne Augmentierung** (`augment: false`, siehe B23) und sind darin nicht mit den Phase-2-Läufen identisch | 04 / 12 | 06, 07 | ✗ | **Neu angelegt 2026-08-21** · Punkt (1) ist ein **eigenes Ergebnis**, keine bloße Einschränkung: Er sagt, dass das Verfahren nicht ausgereizt ist. Punkt (3) ist der eine Einwand gegen die Genauigkeitszahlen, den ein Prüfer selbst finden würde. Umfang: ein Absatz |
...
| F60 | **Die Chefer-Ablation beantwortet den Zirkelschluss-Einwand.** Der bivariate AttnLRP-Pfad ist genau die Größe, auf die das Regularisierungstraining optimiert (F25g); eine Verbesserung dort ist nicht selbsttragend. Chefer et al. (ICCV 2021) teilt keine Berechnung mit diesem Verlust. **Ergebnis (vorläufig, Demo-Split, 17 Clips):** `ratio_over_chance` steigt nach der Regularisierung bei **beiden** Methoden signifikant — AttnLRP +6,30, Chefer +0,848, beide p = 0,0003 | 04 / 12 | 04, 06 | ✗ | **Neu angelegt 2026-08-21** · **Das ist der methodisch wichtigste Teil des ganzen Blocks** und hängt an F25d wie ein Kontrollversuch an einem Ergebnis. Vorbehalt zwingend mitnennen: 17 Clips gegen die 624 der Hauptmessung — vorläufig. Vollständige Herleitung und die 13 Pflicht-Erwähnungen: [`../chefer_ablation.md`](../chefer_ablation.md) §12 |
...
| **P0** | Chefer-Ablation als Gegenprobe | F60, F61, F57 | Beantwortet den Zirkelschluss-Einwand gegen F25d. **Zugleich der Grund, warum F57 erneut `!` ist** — der Beleg sagt inzwischen an vier Stellen, Rollout sei nicht implementiert. |

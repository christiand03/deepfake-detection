...
> **Nachtrag 2026-08-21 — zehn Commits, 48 neue Dateien.** Der Bereich `b9db3f5` …
> `ce2075d` bringt die **Relevanz-Regularisierung** (2026-08-16/17) und die
> **Chefer-Ablation** (2026-08-20) in den Code. Die Registerdokumente
> [00](00_inventar.md), [01](01_datenpipeline.md), [02](02_modelle.md),
> [03](03_training_evaluation.md), [04](04_xai.md), [09](09_tests.md),
> [10](10_konfiguration.md) und [12](12_dokumentation_vault.md) sind nachgezogen
> ([06](06_backend_api.md), [07](07_inference_pipeline.md) und [08](08_frontend.md) waren
> es bereits mit dem Chefer-Commit selbst). Für diese Matrix bedeutet das:
>
> - **F25b ist der Statuswechsel des Registers.** Die Zeile führte
>   Explanation-Guided-Training als *geplant, nicht implementiert*. Das gilt nicht mehr;
>   die Zeile ist unten umgeschrieben, und F25d ist als Ergebniszeile neu.
> - **Neu angelegt:** A48, B22–B25, C20–C22, D50–D54, E13, F58–F64, Q38–Q41.
> - **F57 bleibt `!`, aber aus dem umgekehrten Grund als bisher** — der Beleg sagt jetzt
>   an vier Stellen, Rollout sei *nicht* implementiert, und ein
>   gradienten-gewichtetes Rollout ist es doch. Der Nachtrag steht bereits in der Zeile.
> - ~~**Die Statusspalte der neuen Zeilen ist überwiegend `✗`.**~~ **Überholt am
>   2026-08-24:** Die Blöcke sind am 2026-08-21 in dieselbe Runde geschrieben worden, nur
>   die Statusspalte blieb stehen. Sie steht jetzt auf `○`; die Fundstellen je Kapitel
>   nennt die Statuskorrektur unten.

> **Nachtrag 2026-08-24 — die Chefer-Ablation ist ausgewertet.** Die Commits `7f0e507`
> und `c1dec87` bringen keine neue Funktionalität, sondern **Ergebnisse**: die
> 2 × 3-Methodenablation auf dem test-Split (911 maskierte Chunks aus 624 Clips),
> abgelegt in `docs/results/relevance_method_ablation.csv` und `…_tests.csv`, erzeugt von
> `scripts/build_method_ablation.py` und ausgewertet in
> [`../chefer_ablation.md`](../chefer_ablation.md) §9.3. Für diese Matrix:
>
> - **F60 trägt jetzt die endgültigen Zahlen.** Der Vorbehalt „17 Clips, vorläufig" ist
>   ausgeräumt; die Vorabmessung replizierte bei 37-fachem Stichprobenumfang.
> - **Neu angelegt: F65** (Pointing Game als Leitzahl — die Gegenprobe bestätigt die
>   Richtung uneingeschränkt, die Höhe des Massengewinns nicht) und **F66** (die
>   bivariate und die `fake`-Zahlenreihe dürfen nicht in dieselbe Tabelle).
> - **F25d und F57 sind fortgeschrieben**, nicht umgeschrieben: der Kontrollbefund ist
>   methodenunabhängig geworden, und die beiden gestrichenen Vergleichstafeln haben jetzt
>   belastbare statt vorläufige Zahlen.
> - Nachgezogen wurden [03](03_training_evaluation.md), [04](04_xai.md),
>   [09](09_tests.md) und [12](12_dokumentation_vault.md).

> **Statuskorrektur 2026-08-24 — 33 Zeilen standen zu Unrecht auf `✗`.** Der Nachtrag vom
> 2026-08-21 hat die Punkte der Relevanz- und Chefer-Strecke am selben Tag als
> Modus-A-Blöcke in die Kapiteldateien überführt; die Statusspalte wurde dabei **nicht**
> mitgeführt. Eine Suche nach den Zeilen-IDs in `docs/kapitel/*.tex` findet für jede dieser
> Zeilen einen Block: `04Methodology.tex:2292--2590` (16), `05Experimental_Setup.tex:3214--3490`
> (13), `06Results.tex:1683--1890` (9), `07Discussion_Limitations.tex:732--834` (4),
> `09Appendix.tex:1453--1560` (7), `08Conclusion.tex:265` (1). Die Zeilen stehen deshalb
> jetzt auf `○`.
>
> **Zwei stehen auf `✓`:** F25d und F64 sind in `06Results.tex:641--742` ausgeschrieben —
> `\subsection{Lokalisierung der Relevanz}` mit `tab:results-loc`, zwei
> Abbildungsplatzhaltern und der FF1-Antwort. Bei fünf weiteren Zeilen ist **ein** Kapitel
> geschrieben und ein anderes offen (D52, E13, F25b, F25e, F63); sie stehen auf `○`, weil
> der niedrigste Stand über alle genannten Kapitel gilt, und tragen den Hinweis in der
> Zeile.
>
> **Damit ist `✗` = 0 keine Schönung mehr, sondern der tatsächliche Stand.** Die
> Zusammenfassungstabelle unten meldete das bereits vor dieser Korrektur — sie war der
> Zeilenwahrheit voraus, nicht umgekehrt. **Nicht nachgezogen ist die Verteilung je
> Abschnitt** (§*Die Verteilung je Abschnitt zeigt, wo die Arbeit steht*): Ihre Zahlen
> stammen vom 2026-08-06 und sind vor einer Verwendung neu zu zählen.
...
**Gesamtbestand: 355 Zeilen** (F65 und F66 am 2026-08-24 aus der Chefer-Auswertung
ergänzt), dazu die nicht mitgezählte Checkliste V1–V8.
**Damit ist die Vollständigkeitskontrolle über alle zwölf Registerdokumente
abgeschlossen.**

| Status | Zeilen | Anteil | Bedeutung für die Arbeit |
|---|---:|---:|---|
| `✓` | 45 | 13 % | steht korrekt im Beleg (F25d, F64 und F66 am 2026-08-24 ergänzt) |
| `~` | 0 | 0 % | steht drin, aber unvollständig oder ungenau |
| `○` | **290** | **82 %** | in der Skizze erkannt, noch nicht ausgeschrieben (F65 zu, F25d, F64 und F66 ab) |
| `✗` | **0** | **0 %** | **taucht nirgends auf — auch nicht in einer Skizze.** Seit dem 2026-08-24 auch zeilenweise wahr, s. Statuskorrektur oben |
...
> Schultern und Hintergrund) und **F57** (Attention Rollout als Baseline — seit 2026-08-20 **wieder offen**, weil mit Chefer et al. inzwischen ein gradienten-gewichtetes Rollout implementiert ist) sind im
...
>
> **Historisch — die `✗`-Zahl stieg zwischenzeitlich wieder** (Stand 2026-08-06), von 7 über 14 und 21 auf 29. Das ist
...
> **Fortschreibung 2026-08-07 (Modus B, `00Abstract.tex` — alle vier Skizzenpunkte).**
> Der Skizzenblock des Abstracts ist ausgeschrieben: 12 der 15 aufgenommenen Positionen
> stehen im Fließtext, 2 sind als geprüft-und-nicht-vorhanden abgehakt (Pearson-r,
> Mundregions-Verschiebung), 1 bleibt offen (der Beitragssatz aus der Kopfzeile 8 — nicht
> Teil des Auftrags). Der Abstract hat jetzt fünf Absätze statt drei.
>
> **Keine Statuszelle hat gewechselt, und das ist kein Versehen.** Die Spalte *Kap.*
> nennt für **keine** der 353 Zeilen das Kapitel `00`. Das Abstract ist im Register
> schlicht nicht vorgesehen — es referiert, was in den Kapiteln 4 bis 8 steht, und trägt
> keinen eigenen Mechanismus. Wer den Fortschritt an der Bilanz abliest, sieht diesen
> Durchlauf nicht; die Bilanz ist deshalb **unverändert** und nicht etwa nicht nachgezogen.
>
> **Fünf V-Auflagen sind erstmals praktisch angewandt worden** — im Abstract, nicht in
> `06Results.tex`, für das sie geschrieben sind. Ihre Statuszelle bleibt deshalb `offen`
> und trägt nur einen Vermerk: **V2** hat einen Satz gekostet (der Halbsatz
> \enquote{übertrifft die unimodale Videobasis um [X,X Prozentpunkte]} ist **ersatzlos
> entfallen**, nicht gefüllt worden — unimodale und multimodale AUC sind verschiedene
> Labelaufgaben); **V3** ist für das Abstract zugunsten des aggregierten Werts **0,730**
> entschieden; **V7** steht als Einzelfallmessung mit ausgeschriebenem $n=1$; **V8** ist
> eingehalten, es steht ausschließlich Test-AUC im Text; **V1** kommt nachweislich nicht
> vor.
>
> **Drei Beobachtungen aus dem Formulieren, die vorher nicht sichtbar waren:**
>
> 1. **Der \enquote{Breaking Point} des Abstracts war eine Zahl, die es nicht gibt.** Die
>    alte Fassung versprach \enquote{CRF = [XX] als Breaking-Point-Schwelle, ab dem die
>    Erkennungsrate einbricht}. Gemessen ist keine Schwelle, sondern die **schlechteste
>    Gitterzelle** (CRF 51 / 10\,fps) und ein qualitativer Verlauf (\enquote{steil jenseits
>    CRF 35}, `phase3-robustness-social-media-sweep.md:61`). Der Platzhalter war nicht
>    füllbar — er war falsch gestellt. Dieselbe Formulierung steht als Erwartung auch in
>    der Kopfzeile 7 des Abstracts und in `01Einleitung.tex:10` (FF3 fragt nach einem
>    Schwellwert). **Beim Ausschreiben von FF3 in Kapitel 6/7 ist zu entscheiden, ob die
>    Frage nach einem Schwellwert überhaupt beantwortbar ist** oder ob sie auf
>    \enquote{schlechteste Gitterzelle} umgestellt wird.
> 2. **Die Ergebniszahlen stehen jetzt zum ersten Mal in einer `.tex`-Datei.**
>    `01Einleitung.tex` enthält keine einzige davon, `06Results.tex` und
>    `08Conclusion.tex` führen sie in Skizzen. Ab jetzt gilt für 0,730 / 0,976 / 0,999 /
>    0,997 / 0,960 / 0,857 / 0,527 / 0,929 / 0,741 / 0,975 / 0,921 die Regel aus
>    §*Zahlen mit Verfallsdatum*: **eine Stelle je Zahl**, die übrigen verweisen. Das
>    Abstract ist die einzige legitime Doppelung — es darf wiederholen, was Kapitel 6
>    berichtet.
> 3. **Der Demonstrator hat im Abstract keinen Beleganker.** Der geforderte Halbsatz ist
>    geschrieben, aber alle 40 `○`-Zeilen des Abschnitts **S** zielen auf die Kapitel 4
>    bis 9. Solange dort kein Fließtext steht, behauptet das Abstract etwas, das der Beleg
>    nicht ausführt. Das ist zulässig, aber es priorisiert Abschnitt S: er ist die größte
>    `○`-Gruppe und jetzt zusätzlich im Abstract angekündigt.

> **Fortschreibung 2026-08-07 (Modus B, `05Experimental_Setup.tex` §Überblick).** Der
> erste Abschnitt des Setupkapitels ist ausgeschrieben: drei Stichpunkte plus die
> Quellenauflage, alle vier abgedeckt, drei Absätze, keine offene Position.
>
> **Am selben Tag in Modus C überarbeitet.** Der Autor hat die erste Fassung als
> stilistisch nicht authentisch zurückgewiesen; daraufhin ist der Agentenprompt gegen
> eine frühere Arbeit desselben Autors (*Github Repository Agent*, TU Dresden 2026)
> abgeglichen worden. **Drei Prompt-Regeln waren nachweislich falsch** und wirkten
> gegen den Zielstil: die Wortsperre für *signifikant/essenziell/drastisch/maßgeblich/
> erheblich* (der Autor benutzt sie durchgehend), die Vorgabe \enquote{Satzlänge unter
> 30 Wörtern, ein Gedanke pro Satz} (der Autor schreibt 20--40 Wörter und trägt
> Behauptung plus Einschränkung über `zwar … jedoch` in einem Satz) und das pauschale
> Verbot des Nominalstils. Der Abschnitt ist gegen die korrigierten Regeln neu
> formuliert; Inhalt, Zahlen und Positionsabdeckung sind unverändert, die Erstfassung
> steht als `%|`-Block im Markerpaar. **Konsequenz für alle folgenden Abschnitte:**
> Prompt §6c (Satzmuster) ist ab sofort mitverbindlich, nicht nur die Inhaltsregeln.
>
> **Keine Statuszelle hat gewechselt.** Kein einziger der rund 198 NACHGETRAGEN-Einträge
> nennt \enquote{\section{Ueberblick}} als *Zielabschnitt* — der Abschnitt trägt keinen
> Registermechanismus, sondern führt in das Kapitel ein. Auch keine V- oder N-Auflage ist
> berührt: **der Abschnitt kommt ohne eine einzige Zahl aus**, weil die Hardwaredaten
> nach der Kapitelgrenze in §Hardware und Laufzeitumgebung gehören und dort bereits
> stehen.
>
> **Fünf Beobachtungen aus dem Ausschreiben:**
>
> 1. **Die Ankündigungsliste der Skizze war unvollständig.** Sie nennt neun Gruppen und
>    lässt §Laufzeitkonfiguration der xAI-Analyse und des Demonstrators aus — die
>    zwölfte und letzte Section des Kapitels, im Skizzenkommentar als \enquote{optional,
>    kann in den Anhang} markiert. Der Fließtext nennt sie mit; eine Ankündigung, die
>    einen vorhandenen Abschnitt verschweigt, ist für die Prüferin ein Fehler. Wandert
>    der Abschnitt später in den Anhang, ist dieser Satz nachzuziehen.
> 2. **Kapitel 4 §Überblick trägt dieselbe Abgrenzung bereits** (`04Methodology.tex:6-11`,
>    mit `\ref{setup}` in die Gegenrichtung). Der neue Absatz ist bewusst von der
>    05-Seite formuliert und wiederholt die Begründung nicht. Wer Kapitel 4 in Modus C
>    anfasst, darf die Stelle nicht ein zweites Mal ausbauen.
> 3. **Ein Terminologiekonflikt zwischen den Kapiteln:** Kapitel 4 nennt die
>    16-Frame-Einheiten an **zwei** Stellen \enquote{Datenblöcke}
>    (`04Methodology.tex:25` und `:102`), während die Kapitel 5 und 6 durchgängig
>    **Chunk** schreiben — der feste Bezeichner. Kapitel 5 bleibt bei Chunk. **Der
>    Umbenennungsauftrag steht seit dem 2026-08-07 als `%§§`-Marker an beiden
>    Fundstellen selbst**, nicht nur hier: Zeile 25 trägt die Begründung samt Vorschlag
>    für die Ersteinführung (\enquote{16-Frame-Segmente, im Folgenden Chunks}), Zeile 102
>    verweist darauf. Auszuführen in Modus C beim Anfassen von Kapitel 4. Keine eigene
>    Matrixzeile — es ist kein Mechanismus, sondern dieselbe Fehlerklasse wie die
>    übrigen Synonymwechsel.
> 4. **Sieben der zwölf Sections in Kapitel 5 haben kein `\label`.** §Überblick hat jetzt
>    `sec:setup-ueberblick`. Die Ankündigung nennt die übrigen Abschnitte deshalb in
>    Prosa statt per `\ref`; sobald die fehlenden Labels stehen, kann das umgestellt
>    werden. Fehlend sind die Labels von Hardware, Software-Stack,
>    Klassenungleichgewicht, Modellauswahl, Reproduzierbarkeit und
>    Laufzeitkonfiguration.
> 5. **Die Sectionüberschriften des Kapitels sind ASCII-transliteriert**
>    (\enquote{Ueberblick}, \enquote{Ablations- und Diagnostiklaeufe}). Im
>    Kommentarbereich ist das die Konvention, in einer gesetzten Überschrift ist es ein
>    Fehler. §Überblick ist korrigiert, die übrigen bleiben stehen und sind beim
>    Ausschreiben des jeweiligen Abschnitts mitzunehmen.
>
> **Die Vergleichbarkeitsaussage ist vom Autor bestätigt (2026-08-07).** \enquote{Sämtliche
> Trainings- und Evaluationsläufe liefen auf derselben Single-GPU-Workstation} stammt aus
> der Skizze und ist durch [engineering.md §2](../engineering.md) (eine Entwicklungs-GPU)
> und [launch.md](../launch.md) (der Launch-Agent läuft auf dem Desktop-PC mit GPU)
> gestützt. Eine laufweise Gegenprobe an der W&B-Systeminfo ist **ausdrücklich nicht
> nötig**: der Autor hat bestätigt, dass alles auf demselben Rechner lief. Evidenzstufe
> wie bei A47 — Autorenaussage, im Beleg als Setupbedingung formuliert, nicht als
> Messung. Damit ist die Behauptung, auf der die Vergleichbarkeit des ganzen Kapitels
> ruht, abgeschlossen und nicht erneut zu prüfen.
...
| F57 | 04 §Rollout, 06, 08, 09 | Attention Rollout als **Vergleichsbasis/Referenz**, zwei Vergleichstafeln | ⚠️ **WIEDER OFFEN seit 2026-08-20.** Die Erledigung vom 2026-08-06 („wird nicht implementiert“) ist überholt: Chefer et al. (ICCV 2021) **ist** ein gradienten-gewichtetes Attention-Rollout, ist implementiert (`src/utils/chefer.py`) und wird als Vergleichsbasis ausgewertet. **Vier Belegstellen sind erneut anzupassen** — s. Detailzeile |
...
| A11 | Folge daraus: Fake-Klasse macht auf Chunk-Ebene nur ~7–10 % aus | 01 / 02 | 04, 05 | ○ | **Aufgelöst 2026-08-11** (Entscheidung des Autors): 04 §Labels nennt jetzt „etwa 6 bis 7 %" für die getrennten Label, sagt ausdrücklich „auf der Chunk-Ebene" und verweist für Details auf §Klassenungleichgewicht; der `$$`-Marker ist damit beantwortet und entfernt. Die genauen Werte je Spalte (~6 / ~7 / ~10 %) stehen in Kapitel 5 · zuvor: 04 §Labels nennt 5–7 %; Register/Configs: label_video ~6 %, label_audio ~7 %, kombiniert ~10 %. Der eigene $$-Kommentar (04:70) vermutet die Verwechslung Video-/Chunk-Ebene zu Recht |
...
| A14 | Konkrete Splitgrößen 9959 / 861 / 1180 Videos bei ~30 Identitäten | 10 | 05 | ○ | **Aufgelöst 2026-08-11** (Entscheidung des Autors): Es gelten die **gemessenen** Stände. Tabelle 5.4 führt 9.482/1.382/1.471 Videos bei 165 Identitäten aus `data/processed/*_metadata.csv`, der Absatz darunter den früheren Stand mit 32 Identitäten; die Zahlen 9.959/861/1.180 aus dem Kommentar in `conf/preprocess.yaml` sind eine Vorab-Rechnung für ~30 Identitäten und werden im Beleg nicht verwendet · zuvor: **Zahlen weichen ab** |
...
| A24 | LZF- statt gzip-Kompression, mit Lesebenchmark als Entscheidungsgrundlage | 01 | 09 | – | **Gestrichen 2026-08-19 (Autorenentscheidung).** Der selbst notierte Kandidatenstatus ist eingetreten: Durchsatzoptimierung ohne Aussage für einen Leser. Notiz am Modus-A-Block in `09Appendix.tex`; Begründung in `docs/kapitel/offene_entscheidungen.md`, O13. Berichtet wird stattdessen A19 |
...
| A44 | **Guards gegen Überschreiben der Primärdaten:** `_RESERVED_OUTPUTS` in `preprocess_loose_videos.py` und `_resolve_outputs` in `build_demo_subset.py` verhindern, dass ein externer oder Demo-Lauf die Primärsplits überschreibt; beides ist getestet | 01 / 09 | 09 | – | **Gestrichen 2026-08-19 (Autorenentscheidung)**, gemeinsam mit Q32 und Q33: Der Guard-Absatz war ausgeschrieben und ist ersatzlos entfernt worden — er belegt Sorgfalt, bietet einem Leser aber keinen Mehrwert (`offene_entscheidungen.md`, O12). **Korrektur zur Zeile selbst:** Die Bezeichner sind vertauscht — `build_demo_subset.py` führt `_RESERVED_H5`/`_RESERVED_CSV` (L78-79), `preprocess_loose_videos.py` führt `_RESERVED_OUTPUTS` (L69), und `_resolve_outputs` existiert in **beiden** |
...
| A48 | **Manipulationsmasken als eigener Datenbestand.** `scripts/build_manipulation_masks.py` erzeugt aus jedem Fake und seinem gepaarten `real.mp4` eine Per-Frame-Maske der geänderten Pixel und schreibt sie als `{split}_masks.npz`, **zeilengleich zu `h5_index`** — die HDF5-Dateien bleiben bitgleich. Bestand: **5.807 (train) / 889 (val) / 911 (test)** Masken. Nur `fake_video_*`-Varianten mit nichtleeren `visual_fake_segments`; `real_video_fake_audio` ist per Datensatzdefinition ausgeschlossen, weil seine Videospur unangetastet ist | 01 | 04, 05 | ○ | **Neu angelegt 2026-08-21** · Das ist ein **zweiter Ground-Truth-Bestand** neben den Chunk-Labels und gehört als solcher ins Datenkapitel, nicht nur als Fußnote der Methodik. Verwandt mit A3 (Labelableitung) und F25b. Umfang: ein Absatz + die drei Bestandszahlen |
| A49 | **Gate G0 — die Masken sind geprüft, nicht angenommen.** Die Parameter (`abs_threshold` 0,10, `blur_sigma` 1,5, `max_area_frac` 0,08) stammen aus drei dokumentierten Sweeps: Schwelle × Sigma über 28 Clips, Flächenband über 1.955 Masken, Segmentübereinstimmung über 1.964 Masken je Variante. Ergebnis: **86,6 % Abdeckung**, Median-`in_segment_frac` **1,000**, und die Masken legen **58 % ihrer Energie auf den Mund** gegenüber 11,4 %, die das unbehandelte Modell dorthin gibt | 01 | 04, 05 | ○ | **Neu angelegt 2026-08-21** · Ohne diese Zahlen ist jedes Lokalisierungsergebnis angreifbar: Wenn die Maske selbst nicht auf dem Mund liegt, misst die Metrik nichts. **Der Vergleich 58 % ↔ 11,4 % ist der Satz, der das Trainingssignal rechtfertigt.** Umfang: ein Absatz mit Tabelle |
| A50 | **Zwei Maskenentscheidungen sind gegen die naheliegende Alternative gemessen worden.** (1) Die Beschränkung auf das **Gesichtsoval** ist nicht kosmetisch: über 22 Clips liegen **40–54 %** der rohen Differenzenergie außerhalb des Gesichts, weil beide MP4s unabhängig kodiert sind; die Ovalmaske hebt den Mundanteil von 27 % auf 61 %. (2) `min_in_segment_frac` ist auf **0 (aus)**, weil über 1.964 Masken keine Schwelle gleichzeitig die 80-%-Abdeckungsgrenze hält und alle `real_video_fake_audio`-Chunks entfernt — diese werden stattdessen **über die Variante** ausgeschlossen | 01 | 04 | ○ | **Neu angelegt 2026-08-21** · Beides sind Negativbefunde mit Zahl — genau die Sorte Beleg, die eine Methodik glaubwürdig macht. Umfang: zwei Halbsätze |
| A51 | **Variantenasymmetrie im Datensatz, gemessen bei G0.** Für dasselbe Segment tragen `fake_video_fake_audio` und `fake_video_real_audio` denselben visuellen Edit — aber nicht immer dieselbe Datei: auf 2 von 6 geprüften Segmenten waren die Masken bitgleich, auf den übrigen 3 war die `_fake_audio`-Variante 2–25× größer und ihre stärkste Region wanderte vom Mund weg (Nase/rechtes Auge bei 87–91 %). Ihre Videospur ist separat re-enkodiert; die zusätzliche „Differenz" ist Generierungsrauschen | 01 | 04, 06 | ○ | **Neu angelegt 2026-08-21** · **Eine Datensatzeigenschaft, die über die Maskenerzeugung hinausgeht** — sie betrifft jede Aussage, die `fake_video_fake_audio` und `fake_video_real_audio` zusammenfasst, also auch die kategorienweise Test-AUC (E-Sektion). Vor jeder Aggregation nach `variant` gruppieren. Umfang: ein Absatz · **Kapitel-6-Teil ausgeschrieben 2026-08-24** als Absatz „Variantenasymmetrie" an der kategorienweisen Tabelle der Phase 1 (`06Results.tex`), weil dort die Zellen `visual` und `both` zuerst nebeneinanderstehen. Der Kapitel-4-Teil steht weiter als Block in `04Methodology.tex:2354` |
...
| B22 | **Frames und Maske teilen sich eine Augmentierungsziehung.** `augment_video_frames` ist in `sample_video_augment_params` + `apply_video_augment` aufgespalten; die Maske bekommt über `apply_geometric_augment` **dieselben** geometrischen Parameter. Der Grund ist eine stille Fehlerklasse: ein gespiegelter Frame mit ungespiegelter Maske brächte dem Modell bei, die Manipulation liege auf der anderen Gesichtshälfte — der Verlust bliebe endlich, die Formen passten, nichts scheiterte | 01 | 04, 05 | ○ | **Neu angelegt 2026-08-21** · Dieselbe Klasse wie F12 (SDPA/Eager) und B10: ein Fehler, der ohne Absicherung *lautlos* ein falsches Ziel optimiert. `tests/test_augment_mask_alignment.py` weist die Gleichläufigkeit nach, und ein eigener Test sichert, dass der geseedete Ausgang nach dem Refactor **unverändert** ist. Umfang: ein Absatz |
| B23 | **Der Random-Resized-Crop wird auf maskierten Chunks nicht nachgespielt** (`mask_allow_scale_crop: false`): Die Cropseite liegt bei 12,6–14,0 Zellen des 14×14-Maskengitters, das Nachspielen kostet also bis zu **eine ganze Zelle** (~7 % des Bildes) — mehr, als eine typische Mundmaske groß ist. Die Spiegelung ist bei jeder Auflösung exakt und wird immer nachgespielt. In allen sieben Relevanz-Experimenten ist die Augmentierung ohnehin ganz aus (`augment: false`) | 01 / 10 | 05 | ○ | **Neu angelegt 2026-08-21** · Eine Auflösungsgrenze derselben Art wie F30 (~20 ms im Audio). Wichtig für die Ehrlichkeit des Setups: Die Relevanzläufe trainieren **ohne** Augmentierung, unterscheiden sich darin also von den Phase-2-Läufen — beim Vergleich der Genauigkeitszahlen zu nennen. Umfang: ein Halbsatz + eine Setup-Zeile |
| B24 | **`mask_oversample` gewichtet maskierte Chunks von ~6 % auf ~50 % je Batch** (inverse Häufigkeit über „trägt eine Maske"). Ohne das feuerte der Lokalisierungsverlust auf etwa einem von zwanzig Samples und sein Gradient verschwände neben dem Klassifikationsterm. **Nebeneffekt:** Masken tragen ausschließlich Fake-Chunks, das Übersampeln balanciert die Klassen also implizit mit — deshalb setzen alle Relevanz-Configs `balanced_sampling: false` | 01 / 10 | 05 | ○ | **Neu angelegt 2026-08-21** · Verwandt mit B4/B5 (Sampler ↔ Klassengewichte nicht doppelt korrigieren); hier ist es dieselbe Regel für eine dritte Korrekturquelle. Umfang: ein Halbsatz |
| B25 | **Der Maskenzweig ist rückwärtskompatibel per Konstruktion.** `mask_dir: null` (Vorgabe) lässt jede Phase-1- bis Phase-4-Konfiguration unverändert; unmaskierte Chunks laufen byte-für-byte den alten Pfad; fehlende Maskendateien schalten die Funktion mit **einer** Warnung ab, eine Längenabweichung zwischen Speicher und HDF5 wirft dagegen (sie hieße: anderer Preprocessing-Lauf). Die Zusatzfelder sind für Chunks ohne Maske nullgefüllt, damit die Batchform konstant bleibt und der Default-Collate weiter greift | 01 | 05 | – | **Neu angelegt 2026-08-21**, zugleich **bewusst weggelassen**: Implementierungshygiene. Aufgenommen, damit die Prüfung dokumentiert ist — und weil daraus folgt, dass die Phase-1/2-Ergebnisse durch diese Änderung **nicht** entwertet sind. Das ist der einzige belegrelevante Teil und passt als Halbsatz zu B23 |
...
| C12 | `wav2vec2_module.py` ist als **einziges** der drei Modelle durchgängig `@beartype` + jaxtyping annotiert — **Laufzeitprüfung** der Tensorformen auf `__init__`, `forward`, `model_step`, allen Steps und `explain` | 02 / 11 | 04, 09 | – | **Gestrichen 2026-08-16 (Autorenentscheidung), auch in abgeschwächter Form.** Die Ungleichverteilung geht auf eine Unachtsamkeit in der Arbeitsteilung zurück, beantwortet keine Forschungsfrage und stellt einen Beteiligten schlecht dar. Geschrieben ist allein Q19, die daraus folgende Linterausnahme; der Belegtext sagt über die beiden anderen Modellmodule **nichts**. Begründung: `offene_entscheidungen.md`, O11. Verifiziert 2026-08-16: jaxtyping und `@beartype` kommen ausschließlich in `wav2vec2_module.py` vor (12 bzw. 7 Fundstellen) |
...
| C20 | **`LocalizationHead` — 3.074 Parameter über den Encoder-Tokens.** `LayerNorm → Dropout → Linear(768, 2)`; sagt **zwei** Logits je Token vorher und entfaltet sie auf die 16 Frames, statt 8 vorherzusagen und zu duplizieren — sonst wären die beiden Frames eines Tubelets konstruktionsbedingt ununterscheidbar, und Lippenbewegung ändert sich bei 25 fps zwischen Nachbarframes messbar. Ausgabe auf demselben 14×14-Gitter wie Maske und Verlust, also ohne Resampling dazwischen | 02 | 04 | ○ | **Neu angelegt 2026-08-21** · Die einzige Architekturerweiterung des Projekts seit der Cross-Attention-Fusion. Sie gehört in dieselbe Reihe wie C4 (Fusionskopf), nicht in eine Fußnote. Umfang: ein Absatz + Parameterzahl |
| C21 | **Der Aux-Verlust reitet auf demselben Vorwärtspass mit.** `output_hidden_states` wird **nur** angefordert, wenn der Kopf aktiv ist; ein zweiter Forward entsteht nicht. Der Verlust ist eine **maskierte** BCE nur über die gegateten Frames, mit aus dem Batch abgeleitetem `pos_weight` (`negatives/positives`, auf `[1; 100]` geklemmt), weil die Masken rund 1 % des Gitters abdecken | 02 | 04 | ○ | **Neu angelegt 2026-08-21** · Das Gating ist der Punkt, der begründungspflichtig ist: Frames außerhalb der Segmente mitzuzählen brächte dem Kopf bei, auf den ~11 von 16 echten Frames je Chunk „nichts manipuliert" vorherzusagen. Umfang: ein Halbsatz |
| C22 | **Die gesamte Lokalisierungsstrecke liegt in `VideoMAEModule`, nicht in `base_module.py`.** Weder das Audio- noch das multimodale Modul hat einen Lokalisierungszweig, einen Aux-Kopf oder `explain_chefer` | 02 | 04 | ○ | **Neu angelegt 2026-08-21** · Gegenstück zu C9/C19: Die Querschnittstabelle in [02](02_modelle.md) darf im Beleg nicht suggerieren, die Regularisierung sei modalitätsübergreifend verfügbar. **Konkrete Folge:** Es gibt keine multimodale Lokalisierungsmessung und kann sie ohne Weiteres auch nicht geben — als Limitation zu nennen. Umfang: ein Halbsatz |
...
| D50 | **Explanation-Guided-Training läuft unter manueller Optimierung — und das ist eine Speicherentscheidung.** Unter automatischer Optimierung hält der summierte Verlust den CE-Graphen am Leben, während der Double-Backprop-Graph seinen Höchststand erreicht; die beiden Spitzen **addieren sich** und passen nicht in 8 GB. Manuell gestept wird der CE-Graph zuerst freigegeben, die Spitzen werden sequenziell. Folgekosten: Lightning verbietet unter manueller Optimierung sowohl `trainer.gradient_clip_val` als auch `accumulate_grad_batches`, beides übernimmt das Modul (`grad_clip_val`, `loc_accumulate_grad_batches`), und die Scheduler muss es **intervalltreu** selbst treiben | 02 / 10 | 05 | ○ | **Neu angelegt 2026-08-21** · Dieselbe Klasse wie D14 (`accumulate_grad_batches` bei Batchgröße 1–2): eine Hardwaregrenze, die die Trainingsform bestimmt. Gehört ins Setup-Kapitel, nicht in die Methodik. Umfang: ein Absatz |
| D51 | **Das 8-GB-Budget ist gemessen, nicht geschätzt.** Gate G2 (`scripts/smoke_relevance_backprop.py`) maß **7,57 GB Spitze bei 0,85 s/Schritt** für `attnlrp` gegen 7,81 GB / 1,33 s für `ixg` — die LRP-Regeln kürzen den Rückwärtsgraphen, `attnlrp` ist also **schneller und kleiner** als der vermeintliche Fallback. Daraus folgt `loc_max_samples: 1` als **harte Grenze**, nicht als vorsichtige Vorgabe: Batch 2 im Double-Backprop läuft OOM. Der Spill-Detektor des Gates ist nötig, weil Windows/WDDM bei Überschreitung nicht wirft, sondern still in den geteilten Speicher auslagert und ~9× langsamer wird | 04 / 10 | 05 | ○ | **Neu angelegt 2026-08-21** · **Der Befund „AttnLRP ist billiger als Input×Gradient" ist kontraintuitiv und berichtenswert** — er dreht die übliche Erwartung um. Umfang: ein Absatz mit den vier Zahlen |
| D52 | **`loc_lambda: 0` ist ein echter Kontrollarm, kein Ausschalter.** Bei λ = 0 wird die Relevanz mit `create_graph=False` berechnet und geloggt, erreicht die Gewichte aber nicht; der Lauf hat damit die Trajektorie eines gewöhnlichen CE-Finetunings **und** eine Lokalisierungsspur. `tests/test_attnlrp_patch_scope.py` weist beide Richtungen nach (`create_graph=False` erreicht die Gewichte nicht, `True` schon) | 02 / 09 | 05, 06 | ○ | **Kapitel-6-Teil geschrieben** (`06Results.tex:657-661`), Kapitel-5-Teil als Block in `05Experimental_Setup.tex:3460` · **Neu angelegt 2026-08-21** · Ohne diese Zeile wirkt der Kontrolllauf wie ein weggelassenes Experiment statt wie ein konstruierter Vergleich. **Er trägt das Hauptergebnis** (siehe F25d). Umfang: ein Absatz |
| D53 | **λ wird linear hochgefahren** (`loc_warmup_steps: 200`), weil der Lauf warm bei `val/auc 1.000` startet, wo der CE-Gradient nahezu null ist — ein ungerampter Strafterm wäre ab Schritt eins das *gesamte* Trainingssignal | 02 | 05 | ○ | **Neu angelegt 2026-08-21** · Verwandt mit D6 (Warm-Start) und D18 (Warmup im LR-Plan); hier warmt nicht die Lernrate auf, sondern der Strafterm. Umfang: ein Halbsatz |
| D54 | **`RelevanceCollapseGuard` als Trainingswächter**, mit zwei Achsen: EMA von `loc/mass_total` gegen ihren Startwert (Kollaps der Relevanz) und `val/loss` gegen seinen ersten Wert (Kollaps der Klassifikation). Überwacht bewusst `val/loss` und **nicht** `val/auc`, das bei 1,000 keinen Spielraum mehr hat und lange festgenagelt bleibt, nachdem die Entscheidungsmarge zusammengebrochen ist | 03 / 10 | 05, 09 | ○ | **Neu angelegt 2026-08-21** · Gehört in dieselbe Reihe wie Q10 (Silent-Failure-Audit): eine gebaute Absicherung gegen einen Fehler, der sonst als Erfolg gemeldet würde. **Der Wächter hat zunächst selbst einen Lauf zerstört** (Referenz aus der Sanity-Check-Validierung, `3,0 × 0 = 0`); das ist in Kapitel 7 mehr wert als in Kapitel 5. Umfang: ein Absatz |
...
| E13 | **Fünf Lokalisierungsmetriken, alle in `src/utils/localization.py` definiert und über `scripts/eval_localization.py` erhoben:** `rma` (Anteil der Relevanzmasse in der Maske), **`ratio_over_chance`** (`rma` geteilt durch den Flächenanteil der Maske — die Leitgröße, 1,0 = die Relevanz ignoriert die Maske), `rma_normalized` (Kontrolle gegen zeitliche Konzentration), `pointing_game` (liegt die stärkste Stelle in der Maske) und `iou` gegen die obersten 10 % der Relevanz. Aggregiert **je Clip, nicht je Chunk**, mit 95-%-Perzentil-Bootstrap über Clips (2.000 Ziehungen, `seed=42`) | 04 | 05, 06 | ○ | **Neu angelegt 2026-08-21** · **Das ist die erste quantitative Metrik für die xAI des Projekts überhaupt** — bis hierhin war jede Aussage über die Heatmap qualitativ. Sie gehört als eigener Unterabschnitt neben AUROC und `RecallAtFixedFPR` ins Metrikkapitel; `ratio_over_chance` braucht dieselbe Definitionssorgfalt wie E4. **Ohne die Chance-Normierung ist RMA bedeutungslos:** 0,30 ist hervorragend gegen eine 5-%-Maske und schlecht gegen eine 40-%-Maske. Umfang: ein Unterabschnitt mit Formel · **Stand 2026-08-24:** `06Results.tex:646-651` definiert `ratio_over_chance` und das Pointing Game **behelfsweise am Ort des Ergebnisses**, weil Kapitel 5 den Zielabschnitt noch nicht hat. Der Metrikunterabschnitt in Kapitel 5 steht als Block in `05Experimental_Setup.tex:3247`; sobald er geschrieben ist, wird die Definition in Kapitel 6 durch einen `\ref{}` ersetzt (O24) |
...
| F25b | **Explanation-Guided-Training mit Frame-Difference-Masken** — ✅ **implementiert, gelaufen und ausgewertet (2026-08-16/17)** | 01 / 02 / 04 / 10 | **04 + 06** | ○ | ~~geplant, nicht implementiert~~ — **Statuswechsel 2026-08-21.** Die Zeile führte den Punkt als Vorhaben; das gilt nicht mehr. Implementiert sind: die Maskenerzeugung ([01](01_datenpipeline.md), A48–A51), der skaleninvariante Verlust ([04](04_xai.md)), der Trainingszweig unter manueller Optimierung ([02](02_modelle.md), D50–D54), sieben Experimentkonfigurationen ([10](10_konfiguration.md)) und 14 Testmodule ([09 §8](09_tests.md)). **Die frühere Verzweigung „04+06 *oder* 08" ist entschieden: Methode nach `04Methodology.tex`, Ergebnis nach `06Results.tex`.** 06/07/08 führen den Punkt weiterhin als *geplant/Ausblick* — **alle drei Stellen sind zu korrigieren**, sonst behauptet der Beleg, das eigene Hauptergebnis stehe noch aus · **Stand 2026-08-24: Kapitel 6 ist erledigt** — `06Results.tex:737-742` beantwortet FF1 in beiden Teilen. Offen bleiben der Methodikteil (Block in `04Methodology.tex:2389`) und die Ausblickstelle (Block in `08Conclusion.tex:265`) |
| F25c | Zwei Punkte zu F25b gehören **unabhängig vom Ausgang** in den Beleg: die methodische Spannung eines Explanation-Guided-Loss (Prior *auf die Erklärung* — von „entdecken, warum" zu „vorschreiben, wohin"; Ross et al. 2017 dafür, konstruierte statt entdeckte Erklärung dagegen) und der Trade-off **Lokalisierung ↑ vs. Accuracy ↓** als eigenes Ergebnis | 12 | 04, 07 | ○ | 07 §FF1 fordert die Spannung explizit ein · **Nachtrag 2026-08-21: beide Punkte sind jetzt beziffert.** Der Trade-off ist keine Erwartung mehr, sondern eine Kurve (F25d), und die Spannung ist dadurch schärfer geworden, nicht milder — das Modell *folgt* dem Prior messbar |
| F25d | **Das Ergebnis der Relevanz-Regularisierung**, 911 Chunks aus 624 Test-Clips, alle Arme schrittgleich bei Batch 6.000: Baseline 1,921 · **Kontrolle λ=0 → 1,867** · Aux-Head 2,200 · λ=0,02 → 8,210 · λ=0,1 → 11,418 (`ratio_over_chance`), bei `val/auc_video` 1,0000 / 1,0000 / 0,9953 / 0,9854 / 0,9444. Beide λ>0-Arme haben Konfidenzintervalle, die die Kontrolle nicht überlappen | 04 / 12 | **06** | ✓ | **Neu angelegt 2026-08-21, ausgeschrieben** in `06Results.tex:663-690` (`tab:results-loc`, fünf Arme mit Bootstrap-Intervall und `val/auc_video`) · **Der wichtigste Einzelwert ist 1,867**: Die Kontrolle liegt mit demselben Trainingsbudget, aber ohne Strafterm, *nicht über* der Baseline. Weitertrainieren allein bringt keinen Lokalisierungsgewinn — der gesamte Effekt ist dem Strafterm zuzurechnen, ohne Abschlag für „hat halt länger trainiert". Genau diese Frage stellt ein Betreuer zuerst, und sie ist beantwortet. Rohwerte zitierfähig aus `docs/results/` (nicht aus dem Vault, dort fehlen sie). Umfang: Tabelle + zwei Absätze · **Nachtrag 2026-08-24: der Kontrollbefund ist jetzt methodenunabhängig.** Auch Chefer setzt die Kontrolle unter die Baseline (0,976×, p = 2,8e−81, F60) — das Argument hängt nicht mehr an der optimierten Größe allein. Beim Zitieren die Modusspalte beachten: die 1,867 und 8,210 stammen aus dem `fake`-Arm, die Ablation misst den bivariaten (F66) |
| F25e | **Drei Vorbehalte, die mit F25d zusammen genannt werden müssen.** (1) **Nicht auskonvergiert:** `docs/results/training_curve.csv` zeigt, dass die Lokalisierung bei Batch 6.000 noch *beschleunigt* (λ=0,02: +0,774 → +1,800 je 1.000 Batches), während die Kontrolle exakt flach bleibt (−0,000/1k) — die Zahlen sind **untere Schranken** eines abgeschnittenen Laufs. (2) **Einzelläufe**, `seed=42`, keine Seed-Varianz. (3) **Die Läufe trainieren ohne Augmentierung** (`augment: false`, siehe B23) und sind darin nicht mit den Phase-2-Läufen identisch | 04 / 12 | 06, 07 | ○ | **Neu angelegt 2026-08-21** · Punkt (1) ist ein **eigenes Ergebnis**, keine bloße Einschränkung: Er sagt, dass das Verfahren nicht ausgereizt ist. Punkt (3) ist der eine Einwand gegen die Genauigkeitszahlen, den ein Prüfer selbst finden würde. Umfang: ein Absatz · **Stand 2026-08-24: Kapitel 6 ist geschrieben** (`06Results.tex:692-700`, alle drei Vorbehalte), der Kapitel-7-Teil steht als Modus-A-Block in `07Discussion_Limitations.tex:783` |
| F25f | **Der Anti-Gaming-Nachweis, nicht nur die Behauptung.** Der Verlust ist als **Verhältnis** formuliert (`−log(Masse innen / Masse gesamt)`) und damit invariant unter `R → cR`; die degenerierte Lösung „Relevanz überall gegen null" hat darin **exakt null Gradient**. Das ist analytisch geschlossen, nicht über λ austariert — und die naive Alternative `mean(\|R\|·(1−mask))` aus §7.5 des Quelldokuments wäre zu **null** Klassifikationskosten manipulierbar (Kopf um `c` hoch, letzten Block um `c` herunter: Logits gleich, CE gleich, Strafterm fällt auf `L/c`). Laufzeitnachweis: `loc/mass_total` fiel über die Läufe nur um **23 %**, während sich das Verhältnis verdreifachte | 04 / 09 | 04, 07 | ○ | **Neu angelegt 2026-08-21** · **Ohne diesen Absatz ist F25d angreifbar** — die erste Frage zu einem Lokalisierungsgewinn lautet, ob die Metrik ausgetrickst wurde. Der Nachweis existiert dreifach: analytisch (Modulkopf), im Test (`test_localization_loss.py::test_naive_penalty_is_gameable_and_ours_is_not`) und in der Laufzeittelemetrie. Umfang: ein Absatz mit der Herleitung |
| F25g | **Metrik und Trainingsverlust sind absichtlich dieselbe Größe** (RMA). Die berichtete Zahl ist damit die optimierte Zahl, kein Stellvertreter. Beide arbeiten auf demselben **14×14-Tokengitter**, weil `explain()` seine Relevanz ohnehin dorthin poolt und das anschließende bilineare Upsampling auf 224 ein fester linearer Operator über dieselben 196 Zahlen ist — ein 224er-Verlust wäre eine Umgewichtung bei 256-fachen Kosten | 04 | 04 | ○ | **Neu angelegt 2026-08-21** · Das ist **begründungspflichtig in beide Richtungen**: Es macht die Zahl ehrlich (kein Proxy), lädt aber den Vorwurf ein, man optimiere die eigene Metrik. Genau deshalb existiert die Chefer-Ablation (F60) — die Verbindung gehört in denselben Absatz. Umfang: ein Halbsatz + Verweis |
...
| F58 | **Der lxt-Patch ist prozessweit und dauerhaft — und verändert das *Backward*.** `patch_videomae_for_attnlrp` setzt per `setattr` auf der **Klasse**: `nn.LayerNorm.forward`, `nn.GELU.forward`, `GELUActivation.forward`, `nn.Dropout.forward` und das modulweite `eager_attention_forward`. LayerNorms Varianzpfad wird per `stop_gradient` gekappt, GELUs Ableitung wird zu `GELU(x)/x`, die Attention teilt in **jedem der 12 Blöcke** die Query/Key-Gradienten durch 4 und den Value-Gradienten durch 2. Ein unter Patch zurückpropagierter CE-Verlust liefert daher einen **LRP-Pseudogradienten** statt `dCE/dθ` | 04 | 04 | ○ | **Neu angelegt 2026-08-21** · Für `explain()` (reine Inferenz) harmlos, für Training nicht — ein Verfahren, das einmal beim Start patcht, optimierte still etwas anderes. Deshalb `videomae_attnlrp_patched` als Kontextmanager. **Dieselbe Fehlerklasse wie F12** (SDPA/Eager), nur eine Ebene tiefer, und mit demselben Ausgang: gemessen (`tests/test_attnlrp_patch_scope.py::test_ce_gradient_differs_inside_the_block`), nicht vermutet. Umfang: ein Absatz |
| F59 | **`compute_attnlrp` ist als Trainingssignal unbrauchbar** — es ruft `net.zero_grad()` (löschte die akkumulierten Klassifikationsgradienten) und `target_logits.backward()` (schriebe jedem Parameter den Erklärungsgradienten in `.grad`) und liest dann `x.grad`, einen Blattpuffer **ohne** `grad_fn`. Ein darauf gebauter Verlust hätte **null** Gradient bezüglich der Gewichte: Der Trainingsschritt liefe, konvergierte und änderte nichts. `compute_relevance_differentiable` nutzt stattdessen `autograd.grad(..., create_graph=True)` und fasst keine `.grad`-Puffer an | 04 | 04 | ○ | **Neu angelegt 2026-08-21** · Ein Fehler, der als erfolgreicher Lauf endet — dieselbe Kategorie wie H-Zeile „PGD verschmutzt die Gewichtsgradienten nicht". Der Absatz erklärt zugleich, **warum** die Implementierung eine zweite Relevanzfunktion braucht statt die vorhandene zu benutzen. Umfang: ein Absatz |
| F60 | **Die Chefer-Ablation beantwortet den Zirkelschluss-Einwand.** Der bivariate AttnLRP-Pfad ist genau die Größe, auf die das Regularisierungstraining optimiert (F25g); eine Verbesserung dort ist nicht selbsttragend. Chefer et al. (ICCV 2021) teilt keine Berechnung mit diesem Verlust. **Endgültiges Ergebnis (test-Split, 2026-08-22): 2 × 3 Arme auf denselben 911 maskierten Chunks aus 624 Clips.** `ratio_over_chance` gegen die Kontrolle λ=0: AttnLRP (bivariat) 1,898 → 7,910 (**4,17×**, p = 4,5e−103), Chefer 1,536 → 2,360 (**1,54×**, p = 4,3e−103), gepaarter Wilcoxon über Clips. **Beide** Methoden setzen die Kontrolle unter die Baseline (0,972× bzw. 0,976×; p = 1,2e−26 bzw. 2,8e−81) — Weitertrainieren allein lokalisiert nicht, und dieser Satz steht jetzt auf zwei Verfahren ohne gemeinsame Berechnung statt auf AttnLRP allein | 04 / 12 | 04, 06 | ○ | **Neu angelegt 2026-08-21, Zahlen ersetzt 2026-08-24** · **Das ist der methodisch wichtigste Teil des ganzen Blocks** und hängt an F25d wie ein Kontrollversuch an einem Ergebnis. ~~Vorbehalt: 17 Clips, vorläufig~~ — **erledigt:** die Vorabmessung auf dem demo-Split (25 Chunks, 17 Clips, +6,30 / +0,848 bei p = 0,0003) replizierte bei 37-fachem Stichprobenumfang (4,05× / 1,50× gegen die Baseline) und ist abgelöst. Zitierfähig aus `docs/results/relevance_method_ablation{,_tests}.csv`, nicht aus §9.1. **Die Höhe des Effekts ist damit aber nicht mitbestätigt — siehe F65.** Vollständige Herleitung und die 13 Pflicht-Erwähnungen: [`../chefer_ablation.md`](../chefer_ablation.md) §9.3 und §12 · **Kapitel-6-Teil ausgeschrieben 2026-08-24** (`06Results.tex`, §Lokalisierung der Relevanz, Absatz „Gegenprobe mit einer zweiten Methode" und `tab:results-loc-methoden`); der Methodikteil in Kapitel 4 steht weiter als Block in `04Methodology.tex:2532` |
| F61 | **Zwei benannte Abweichungen vom Chefer-Paper**, beide architektonisch erzwungen: `readout="mean"` statt der CLS-Zeile (VideoMAE hat kein CLS-Token, der Kopf mittelt) und **acht statt sechzehn** Zeitpositionen (`tubelet_size=2`). Zusätzlich wird die Identitätsinitialisierung vor der Ablesung abgezogen (`R − I`) — ohne das legt sie bei `n = 1568` Tokens einen `1/n`-Sockel unter jedes Token: an einem echten Clip gemessen **99 % des schwächsten Wertes**, die Dynamik fällt von 27,6× auf 1,3×, und `ratio_over_chance` kollabierte **konstruktionsbedingt** gegen 1,0 | 04 | 04 | ○ | **Neu angelegt 2026-08-21** · Die Abweichungen sind laut Quelldokument im Beleg **als solche zu benennen** — eine Methode „nach Chefer et al." zu nennen, die anders abliest, wäre eine Falschangabe. Der `R − I`-Punkt ist zusätzlich ein schönes Beispiel dafür, wie eine Metrik still gegen ihren Zufallswert laufen kann. Umfang: ein Absatz |
| F62 | **Chefer läuft zwingend un-gepatcht**, in `lxt_patches_disabled` — sonst wäre `∂logit/∂attention` ein LRP-Pseudogradient und die Methode gäbe eine plausibel aussehende, aber falsche Karte zurück, ohne Fehler und ohne Warnung. Der Kontextmanager stellt die unberührten Forwards **aller** lxt-mutierten Klassen her (auch der Wav2Vec2- und multimodalen Patches, sie mutieren dieselben vier Klassen). Weil die Mutationen prozessweit sind, serialisiert die API seit dem 2026-08-20 alle Modellarbeit über **einen** Executor (`src/api/executor.py`) | 04 / 06 | 04 | ○ | **Neu angelegt 2026-08-21** · Verwandt mit F29 (Idempotenz der Patches) — dort ging es um doppeltes Patchen, hier um paralleles Ent-Patchen. **Verhaltensänderung im Demonstrator:** Robustheits-Sweep und Analyse laufen nun nacheinander statt parallel; das ist im Systemkapitel zu erwähnen. Umfang: ein Halbsatz + Verweis auf S-Sektion |
| F63 | **Der Aux-Kopf ist der aufschlussreichste Negativbefund des Blocks.** Die Encoder-Merkmale direkt auf die Maske zu supervidieren bewegt die AttnLRP-Relevanz **um ein Vielfaches weniger** als die Erklärung direkt zu bestrafen — Faktor 1,18 gegen 4,40 bei λ=0,02, und das bei vergleichbaren Genauigkeitskosten (−0,005 gegen −0,015 AUC). Der Kopf erreicht `aux_iou` 0,069, das Encoder-Signal *enthält* die Ortsinformation also nachweislich; die Attribution folgt ihr trotzdem kaum | 02 / 04 | 06, 07 | ○ | **Neu angelegt 2026-08-21** · **Für eine xAI-Arbeit ist das ein Ergebnis über AttnLRP selbst, nicht nur über dieses Modell:** Heatmap und Merkmalsqualität sind teilweise entkoppelt, eine Erklärung lässt sich wirksamer durch einen Prior *auf die Erklärung* verschieben als durch bessere Repräsentationen. Das **verschärft** die Spannung aus F25c. Vorbehalt: `aux_iou` stieg am Laufende noch, 0,069 ist eine untere Schranke. Umfang: ein Absatz in Kapitel 7 · **Stand 2026-08-24: der deskriptive Kapitel-6-Teil ist geschrieben** (`06Results.tex:714-723`), die Deutung steht als Modus-A-Block in `07Discussion_Limitations.tex:732` |
| F64 | **Der Aux-Kopf hat die gesünderen Trainingskurven und das schwächere Ergebnis.** Bei ihm verbessern sich beide Ziele *gemeinsam* (`val/loss` 0,322 → 0,190, `val/auc_video` 0,975 → 0,999, `aux_iou` 0,048 → 0,069), bei der Regularisierung kämpfen sie gegeneinander — auf der gemeinsamen Metrik liefert der Kopf trotzdem rund ein Fünftel dessen, was λ=0,02 erreicht | 04 / 12 | 06 | ✓ | **Neu angelegt 2026-08-21** als `–` (bewusst weggelassen), **am selben Tag auf `✗` gesetzt**, **ausgeschrieben in `06Results.tex:719-723` samt Bildunterschrift `:731-733`**: Der Autor hat entschieden, die Trainingskurven des Aux-Head-Laufs abzubilden. Damit greift die Bedingung, unter der der Punkt Pflicht wird — ohne den Halbsatz liest die Abbildung sich als Argument *für* den Aux-Kopf, obwohl er auf der gemeinsamen Metrik rund ein Fünftel dessen liefert, was λ=0,02 erreicht. **Die Daten liegen nicht im Repositorium** (kein `logs/`, keine `aux_iou`-Spalte in `training_curve.csv`); die Abbildung entsteht als beschrifteter Platzhalter, siehe `offene_entscheidungen.md` O20 |
| F65 | **Spitze und Masse verhalten sich verschieden — die Leitzahl ist das Pointing Game, nicht die 7,910.** Bei den massenbasierten Metriken ist der AttnLRP-Effekt rund dreimal so groß wie der von Chefer (`ratio_over_chance` 4,17× gegen 1,54×, `rma` 2,91× gegen 1,34×), was zu erwarten ist, weil der Verlust ein Massenverhältnis auf AttnLRP-Relevanz minimiert. Beim **Pointing Game** kehrt sich das Verhältnis um: Chefer zeigt mit 3,37× den größeren relativen Sprung (AttnLRP 2,75×), und die Endwerte liegen mit 0,769 gegen 0,747 praktisch aufeinander, ausgehend von 0,280 bzw. 0,221. Es ist die einzige der vier Metriken auf [0, 1] und damit die einzige, die **ohne Skalenvorbehalt zwischen den Methoden vergleichbar** ist | 04 / 12 | **06, 07** | ○ | **Neu angelegt 2026-08-24** · Die saubere Formulierung lautet: **das Training verschiebt, *wohin* das Modell schaut; die zusätzliche Massenkonzentration ist teilweise AttnLRP-spezifisch.** „Nicht auf LRP overfittet" trifft auf die Spitze uneingeschränkt zu, auf die Masse nur eingeschränkt — wer nur F60 schreibt und diesen Punkt weglässt, überzeichnet die Gegenprobe. Zwei Verfahren ohne gemeinsame Berechnung sind sich einig, dass die stärkste Stelle der Karte nach dem Training in rund drei von vier Fällen in der Maske liegt, vorher in etwa einem von vier. Präzisiert zugleich den Vorbehalt aus §9.1, die Werte seien zwischen den Methoden unvergleichbar: die *Höhen* sind es nicht, die *Verhältnisse* sehr wohl. Umfang: ein Absatz in 06 (Tabelle) und ein Halbsatz in 07. Quelle: `docs/results/relevance_method_ablation_tests.csv`, [`../chefer_ablation.md`](../chefer_ablation.md) §9.3 · **Als Skizze eingetragen 2026-08-24:** `06Results.tex` (Block „F65", gemeinsam mit dem Zahlenstand zu F60) und `07Discussion_Limitations.tex` (Block „F65", Deutung) · **Kapitel-6-Teil noch am selben Tag ausgeschrieben** — `tab:results-loc-methoden` führt beide Metriken, der Folgeabsatz nennt die Umkehr beim Pointing Game und der Absatz danach die Leseregel; die FF-Tabelle `tab:results-ff` führt den Sprung seither in der FF-1-Zeile mit. Die Deutung in Kapitel 7 steht aus |
| F66 | **Es gibt zwei AttnLRP-Zahlenreihen, und sie dürfen nicht in eine Tabelle.** Die Methodenablation misst den **bivariaten** Arm (`ratio_over_chance` 1,953 / 1,898 / 7,910 für Baseline / Kontrolle / λ=0,02), die Ergebnistabelle aus `docs/results/loc_*.json` den **Einzelziel-Arm** `fake` (1,921 / 1,867 / 8,210). Beide sind gültig, gemessen auf denselben 911 Chunks, und stützen sich gegenseitig — es sind aber verschiedene Größen (`\|R_fake\| + \|R_real\|` gegen die reine FAKE-Relevanz) | 04 / 12 | 06 | ✓ | **Neu angelegt und am selben Tag umgesetzt 2026-08-24:** Beide Tabellenunterschriften nennen jetzt ihren Modus — `tab:results-loc` die Single-Target-Relevanz, `tab:results-loc-methoden` die Magnitude, jede mit Querverweis auf die andere · **Eine Zitierfalle für `06Results.tex`:** F25d nennt 8,210, F60 nennt 7,910, und beide Zahlen beschreiben denselben Checkpoint. Ohne die Modusspalte sieht das nach einem Widerspruch aus. Empfehlung: im Beleg **eine** Reihe führen (die bivariate, weil sie die Größe ist, die die Oberfläche rendert und die die Ablation vergleicht) und die zweite in einer Fußnote als Konsistenzprüfung nennen. Der `fake`-Arm wurde für §9.3 nicht neu erhoben; sein Codepfad ist unverändert und eine Stichprobe reproduzierte die gespeicherten Zeilen in allen fünf Metriken auf 0,000e+00. → `offene_entscheidungen.md` **O25, entschieden 2026-08-24: Option B** — Hauptreihe `fake`, die bivariate Reihe nur in der Gegenprobentabelle, **beide Tabellen nennen ihren Modus in der Unterschrift**, kein Nachmessen · **Als Skizze eingetragen 2026-08-24** (`06Results.tex`, Block „F66"). **Nachgeprüft 2026-08-24:** Der Trainingsverlust erklärt `loc_target_class = 1` (`src/models/VideoMAE_module.py:44,308`) — die optimierte Größe ist damit der `fake`-Arm, nicht der bivariate. Das ist das Argument, das O25 entscheidet |
...
| H30 | **Das Trainingsbudget der Phase 4.2 ist mit dem Angriffsbudget der Phase 4.1 vergleichbar:** alle drei Module setzen `adv_epsilon = 0,03` und `adv_steps = 7`, die drei `train_*_adversarial`-Konfigurationen übernehmen dieselben Werte, und die Schrittweite ist überall `ε / steps · 2,5` — dieselbe Heuristik wie in `_pgd_attack` | 05 / 10 | 05 | – | **Bewusst weggelassen 2026-08-11** (Entscheidung des Autors): Der Punkt beschreibt das Trainingsbudget der **Phase 4.2**. Nach H29 wurde der multimodale Adversariallauf nicht ausgeführt und wird es voraussichtlich auch nicht; ohne berichtete Phase-4.2-Zahlen hat die Angabe keinen Bezugspunkt. Wieder aufzunehmen, sobald Phase 4.2 berichtet wird — dieselbe Bedingung wie bei H29 · **Neu angelegt 2026-08-06** · Die 05-Skizze führt das ε-Gitter der **Angriffe** (0,01--0,1), nennt aber kein Trainingsbudget. Ohne H30 ist unklar, ob die Härtung im selben Budgetbereich stattfindet wie der Angriff, gegen den sie schützen soll — sie tut es (0,03 liegt mitten im Angriffsgitter). Verifiziert gegen `train_*_adversarial.yaml:32-34`. Umfang: Tabellenzeile |
...
| H34 | **Im voreingestellten Adversarialsweep sind die ε-Budgets *nicht* getrennt:** ohne `--audio-epsilon` spiegelt das Audiobudget je Gitterpunkt den Video-ε-Wert | 05 | 05, 06 | ✓ | **Eingearbeitet 2026-08-11** in §5.11, vierter Absatz, hinter dem `--attack-modalities`-Satz: „Ohne `--audio-epsilon` übernimmt der Audiostrom je Gitterpunkt den Video-Wert. Vergleichbar sind damit die Modalitäten bei gleichem Budget, nicht bei unabhängig gewählten." Verifiziert: `eval_adversarial_sweep.py:443`, Vorgabe `None` in `:583`. Der Kapitel-6-Anteil (Vorbehalt bei jedem Modalitätenvergleich) bleibt offen · **Neu angelegt 2026-08-06** · H7 führt die getrennten Budgets als Fähigkeit. Im gefahrenen Sweep sind sie in der Voreinstellung gekoppelt — eine Aussage \enquote{Audio ist bei gleichem ε anfälliger} ist damit möglich, eine über *unabhängig gewählte* Budgets nicht. Umfang: Halbsatz an die Sweep-Tabelle |
...
| S3 | Checkpoint-Auswahl über Umgebungsvariablen; fehlender Checkpoint → HTTP 503 | 06 | 04, 09 | – | **Bewusst weggelassen 2026-08-11** (Kürzung von §5.12 durch den Autor). Im Beleg bleibt nur der Kernsatz \enquote{Welcher Checkpoint eine Anzeige bedient, entscheiden allein die Umgebungsvariablen}; die Variablennamen, HTTP 503 und das stille `audio: null` sind gestrichen. **Nachtrag 2026-08-19: auch die Variablentabelle im Anhang entfällt** (Autorenentscheidung) — Betriebskonfiguration ohne Mehrwert für eine Prüferin; Kapitel 9 nennt keine Umgebungsvariablen mehr, die Unterschrift der Befehlstabelle verweist für die Checkpointpfade auf Kapitel 5 (`offene_entscheidungen.md`, O17) · zuvor: nachgetragen 2026-08-05 in die 05-Skizze §Laufzeitkonfiguration (mit S22, ein Absatz: 503, stilles `audio: null`, Zuordnung Fusionsmodus) und die 09-Skizze §G (Variablentabelle). Abweichung von der Kap.-Spalte: Laufbedingung, keine Designbegründung — wie D20/D29 |
| S4 | Cache-Schlüssel kodiert **jeden** einstellbaren Parameter | 06 | 04 | – | **Bewusst weggelassen 2026-08-11** (Kürzung von §5.12). Im Beleg bleibt nur \enquote{Alle Analyseergebnisse werden für spätere Abrufe vollständig gecached}; die Schlüsselbestandteile sind gestrichen · zuvor: nachgetragen 2026-08-05 in die 05-Skizze §Laufzeitkonfiguration (mit S13/S14/S20, ein Absatz) |
| S5 | 20 Pydantic-Schemas als API-Vertrag; TS-Gegenstück **manuell** synchron gehalten | 06 / 08 | 04 | – | **Gestrichen 2026-08-19 (Autorenentscheidung).** Der selbst notierte Kandidatenstatus ist eingetreten: Implementierungshygiene, trifft keine Forschungsfrage (`offene_entscheidungen.md`, O17) |
...
| S11 | Mock-Modus (`VITE_USE_MOCK`) — **Screenshots müssen aus dem echten Backend stammen**. Der Mock deckt **nur** Clipliste und Phase 3/4 ab; `analyzeClip()` hat **keinen** Mock-Pfad, die Hauptanalyse braucht immer ein laufendes Backend | 08 | 05 | – | **Bewusst weggelassen 2026-08-11** (Kürzung von §5.12; der Absatz zu Mock-Modus und Erzeugungsvorschrift ist entfallen) · „Ohne Backend vorführbar" trifft nicht zu · **Screenshot-Fehlerquelle** · nachgetragen 2026-08-05 in die 05-Skizze (§Laufzeitkonfiguration, an die Erzeugungsvorschrift S4/S13/S14/S20). Die Spalte *Kap.* war leer; Zuordnung zu Kapitel 5 mit derselben Begründung wie F24/F31 (Laufbedingung der Abbildungserzeugung) |
| S12 | Kein Frontend-Test vorhanden | 09 | 07, 09 | ○ | Limitation · überführt 2026-08-06 in die 07-Skizze (gemeinsam mit Q21, ein Satz) |
| S13 | **Der Analysecache wird nie invalidiert** — der Schlüssel kodiert Clip und Parameter, **nicht das Modell**. Nach einem Checkpointwechsel liefert derselbe Clip weiter das alte Ergebnis; `data/analysis_cache/` muss von Hand geleert werden | 06 | 04, 07 | – | **Für Kapitel 5 bewusst weggelassen 2026-08-11** (Kürzung von §5.12). **Der Kapitel-7-Anteil bleibt offen:** Punkt 21 der Limitationen ist davon unberührt · Fehlerquelle für Abbildungen · nachgetragen 2026-08-05 in die 05-Skizze §Laufzeitkonfiguration (mit S4/S14/S20) und die 07-Skizze §Limitationen (Punkt 21). Abweichung von der Kap.-Spalte: die Vorschrift dagegen ist eine Laufbedingung, Kapitel 4 bekommt keinen eigenen Eintrag |
| S14 | Auch die Registry- und CSV-Modulcaches werden nie invalidiert: Änderungen an `clips.json` oder den Metadaten-CSVs wirken erst nach einem **Serverneustart** | 06 | 04 | – | **Bewusst weggelassen 2026-08-11** (Kürzung von §5.12) · zuvor: nachgetragen 2026-08-05 in die 05-Skizze §Laufzeitkonfiguration (mit S4/S13/S20) |
...
| S20 | Abwärtskompatible Vorgabewerte (`[]`, `0.0`, `False`, `None`) halten alte Cachedateien gültig — **leere Listen sind als „altes Ergebnis" zu lesen, nicht als „gemessene Null"** | 06 | 04 | – | **Bewusst weggelassen 2026-08-11** (Kürzung von §5.12). Der Lesehinweis \enquote{leere Liste ≠ gemessene Null} entfällt damit im Setup; falls Kapitel 6 eine leere Liste zeigt, gehört er in die Bildunterschrift · zuvor: nachgetragen 2026-08-05 in die 05-Skizze §Laufzeitkonfiguration (mit S4/S13/S14) |
| S21 | Die Parametergrenzen der Labore sind schema-erzwungen (CRF 18–51, fps 5–30, σ 0–50, AAC 8–320 kbps, 0 < ε ≤ 0,5, Schritte 1–100, `fusion_mode` ∈ {cross_attention, concat}); Verletzung ⇒ HTTP 422, die Anfrage erreicht die Inferenz nie | 06 / 08 | 04 | – | **Bewusst weggelassen 2026-08-11** (Kürzung von §5.12; der Absatz zu den Schemagrenzen ist entfallen). **Folge:** Der Vorbehalt \enquote{Screenshot-Zahl ≠ Sweep-Zelle} steht nirgends mehr — falls Kapitel 6 eine interaktiv erzeugte Zahl zeigt, gehört er in deren Bildunterschrift · zuvor: nachgetragen 2026-08-05 in die 05-Skizze §Laufzeitkonfiguration; Abweichung von der Kap.-Spalte wie bei H10/H20/H21. **Nicht identisch mit dem Offline-Gitter** — die Oberfläche lässt Zwischenwerte zu |
| S22 | **Der Fusionsmodus wird geprüft, aber nicht erzwungen**: ein Checkpoint mit abweichendem `fusion_mode` wird nach einer bloßen Logwarnung trotzdem benutzt | 06 / 07 | 04 | – | **Bewusst weggelassen 2026-08-11** (Kürzung von §5.12) · Zuordnung Modus ↔ Umgebungsvariable prüfen · nachgetragen 2026-08-05 in die 05-Skizze §Laufzeitkonfiguration (mit S3). Verifiziert: `src/api/inference.py:184-194` loggt nur eine Warnung und cacht das Modell trotzdem. **Anschluss: S23 (Reg. 07) gehört in denselben Absatz — am 2026-08-05 als Punkt (d) desselben Absatzes nachgetragen** |
| S23 | **Nur `run_multimodal_inference` reicht `fusion_mode` durch.** Alle Phase-4-Ergebnisse und alle multimodalen Sweep-Werte gelten für **`cross_attention`** — unabhängig davon, was im Frontend umgeschaltet ist; einen Concat-Vergleich gibt es dort nicht | 07 | 04, 06 | ○ | **Der Kapitel-5-Anteil ist am 2026-08-11 bewusst weggelassen** (Kürzung von §5.12). Kapitel 4 (Reichweite) und Kapitel 6 (Gültigkeitsangabe zu den Zahlen) bleiben offen und sind jetzt die einzigen Träger dieser Aussage · **Alle Phase-4-Zahlen gelten für `cross_attention`** — ohne diesen Satz droht eine Fehlzuordnung · nachgetragen 2026-08-05 **dreigeteilt**: Reichweite der Phasen 3/4 in die 04-Skizze (§Methodik zur Evaluierung der Robustheit, Vorspann), Laufbedingung in die 05-Skizze (§Laufzeitkonfiguration, Punkt (d) des Absatzes S3+S22), Gültigkeitsangabe zu den Zahlen in die 06-Skizze (§Phase 3 und §Phase 4). Verifiziert: `src/api/inference.py:151` (Vorgabewert `cross_attention`), `:2420` einzige Weitergabe, `:3343/:3615/:3673` Aufruf ohne Argument; `eval_robustness_sweep.py:569,628`, `eval_adversarial_sweep.py:389,453`, `compute_uap.py:388`. **Abgrenzung:** die interaktive Robustheitsroute reicht den Modus durch (`src/api/routers/robustness.py:57`) — die feste Verdrahtung dort ist das Frontend-Gegenstück **S24** (Reg. 08) und weiterhin offen |
| S24 | Im Robustheitslabor ist der Fusionsmodus **fest auf `cross_attention`** verdrahtet; eigenständiger Wav2Vec-Audiotest und Multimodalmodus schließen sich gegenseitig aus | 08 | 05 | – | **Bewusst weggelassen 2026-08-11** (Kürzung von §5.12) · zuvor: nachgetragen 2026-08-05 in die 05-Skizze als Punkt (e) des Absatzes S3+S22+S23 — damit ist die Abgrenzung aus S23 eingelöst: auch die interaktive Route läuft praktisch auf `cross_attention`. Abweichung von der Kap.-Spalte wie bei S3/S22/S23. Verifiziert: `RobustnessPanel.tsx:362`, `:490-498` |
...
| S37 | **Die Laufzeitpipeline schreibt auf die Platte:** die MP4-Crop-Videos der Phase-3/4-Spieler liegen unter `data/phase_media/`, umlenkbar über `PHASE_MEDIA_DIR`. Der Dateiname der Degradations- bzw. Angriffsvideos ist an den Cacheschlüssel gebunden (`media_prefix`), der des sauberen Videos an den Clip-Stamm; beide Ausgaben überdauern den Prozess | 07 / 06 | 05 | – | **Bewusst weggelassen 2026-08-11** (Kürzung von §5.12; der Cacheabsatz, in den der Punkt gehörte, ist auf einen Satz reduziert) · zuvor: überführt 2026-08-06 in die 05-Skizze (§Laufzeitkonfiguration, an den Cacheabsatz) · Die zweite Plattenausgabe (WhisperX-Transkriptcache unter `.whisperx_cache/`) ist bereits über F14 („Plattencache") abgedeckt · gehört in denselben Absatz wie S4/S13/S14/S20 (Cacheverhalten, 05-Skizze §Laufzeitkonfiguration) · **Zeile neu angelegt 2026-08-05** aus [07 §Was `inference.py` nicht tut](07_inference_pipeline.md) und [06 §phase\_media.py](06_backend_api.md) |
...
| Q4 | ~~336~~ **564 Tests; 25 davon weisen methodische Eigenschaften nach** | 09 | 09 | ○ | 09-Skizze G fordert die Zuordnung Test → Silent-Failure-Klasse · **Zahl aktualisiert 2026-08-21**, Begründung in Q38 |
...
| Q16 | Versionierte Reproduktionskette vorhanden: `configs/paths` über `DEEPFAKE_DATA_DIR`/`_LOG_DIR`/`_CKPT_DIR` umlenkbar, `PROJECT_ROOT` per `rootutils.setup_root` — alle Pfade unabhängig vom Arbeitsverzeichnis | 10 / 11 | 05 | ○ | **Zielkapitel 2026-08-19 auf 05 verengt.** Der Anhangsanteil (Variablentabelle in §A) war geschrieben und ist ersatzlos entfernt worden — Betriebskonfiguration ohne Mehrwert für eine Prüferin (`offene_entscheidungen.md`, O17). Für Kapitel 5 bleibt der Punkt **offen**: nachgetragen 2026-08-05 in die 05-Skizze (§Reproduzierbarkeit, an den D20+D34-Absatz); verifiziert gegen `configs/paths/default.yaml:4-19`: drei `oc.env`-Überschreibungen mit Vorgabewert, `root_dir` als einzige ohne |
| Q17 | `torch.set_float32_matmul_precision("medium")` und `add_safe_globals` an **allen sechs** checkpointladenden Einstiegspunkten (sonst scheitert das Entpicklen unter `weights_only=True`) | 03 | 05, 09 | – | **Gestrichen 2026-08-16 (Autorenentscheidung), nicht relevant.** Ladebedingungen der Bibliothek, keine Eigenschaft dieser Arbeit; ein Leser gewinnt daraus nichts. Dieselbe Begründung wie bei den Guards (`offene_entscheidungen.md`, O12/O13). Notiz am Modus-A-Block in `09Appendix.tex` |
...
| Q20 | **Silent-Failure-Klasse in der Konfiguration:** Callback-Einträge **ohne `_target_` werden kommentarlos übersprungen** — ein Lauf kann ohne Early Stopping oder Checkpointing durchlaufen, ohne dass das Log es anzeigt. Gegenmittel ist der mitgeschriebene `config_tree.log` | 03 | 07 | ○ | **Zielkapitel 2026-08-16 von 09 auf 07 geändert.** Als Qualitätsnachweis trägt der Punkt nicht — er belegt keine Sorgfalt, sondern eine Schwäche; als Vorbehalt zur Aussagekraft der Zahlen gehört er zu den Limitationen. Steht als Modus-A-Block am Ende von `07Discussion_Limitations.tex`, vorgesehen als Punkt 24 der Ergänzungsliste und dort **neu zu bewerten**: Kapitel 5 §Modellauswahl benennt bereits die Folge (Test mit den Gewichten der letzten Epoche), Q20 ist die Ursache dazu. Kein Befund, dass der Fall in einem berichteten Lauf eintrat — als Fehlerklasse zu formulieren, nicht als Vorfall (`offene_entscheidungen.md`, O12) |
...
| Q38 | **Die Testzahl ist von 336 auf 564 gestiegen** (38 → 52 Module, 6.085 → 9.152 Zeilen). Die 14 neuen Module gehören sämtlich zur Relevanz-Regularisierung und zur Chefer-Ablation; **12 weitere Tests weisen methodische Eigenschaften nach** und nicht nur Code (siehe [09 §1](09_tests.md)) | 09 | 09 | ○ | **Neu angelegt 2026-08-21** · **Ersetzt die Zahl in Q4.** Wichtiger als der Zuwachs ist die Verschiebung: Der neue Block testet überwiegend *Eigenschaften* (Skaleninvarianz, Gradientenpfade, Patch-Reichweite) statt Ein-/Ausgaben. Das ist für Kapitel 9 die interessantere Aussage |
| Q39 | **Kein Test läuft gegen ein echtes VideoMAE.** Die Relevanz- und Chefer-Tests arbeiten gegen Stellvertretermodelle mit analytisch bekanntem Verhalten. Was nur ein echter Backbone beantworten kann — passt der Double-Backprop in 8 GB, gibt HuggingFace die Attention-Tensoren im Graphen zurück statt als abgelöste Kopien — prüfen stattdessen zwei **Gates** (`scripts/smoke_relevance_backprop.py`, `scripts/smoke_chefer.py`). Die sind keine Tests: Sie brauchen Checkpoint und GPU und laufen nicht in der CI | 04 / 09 | 09 | ○ | **Neu angelegt 2026-08-21** · Ein Ehrlichkeitspunkt derselben Klasse wie die Abdeckungslücken in [09](09_tests.md). **Die Gate-Struktur selbst ist berichtenswert:** beide sind so geschrieben, dass „drei von vier Prüfungen bestanden" ausdrücklich nicht genügt |
| Q40 | **`tests/test_checkpoint_config.py` testet YAML, nicht Code** — und dokumentiert damit zwei verlorene Läufe: `val/auc_video` sättigt bei 1,000, danach löst `save_top_k` nie wieder aus (Run 0 lief 18.000 Batches, Best- und `last.ckpt` froren bei 6.000 ein); und `save_top_k: 2` behält bei *steigendem* `val/loss` die **frühesten** Validierungen, wodurch die λ>0-Arme bei Batch 3.000 und die Kontrolle bei 6.000 ausgewertet wurden. Die erste Fassung der Trade-off-Kurve mischte deshalb Punkte unterschiedlicher Trainingsdauer | 09 / 10 | 05, 09 | ○ | **Neu angelegt 2026-08-21** · **Der Fehler ist im Ergebnis nicht sichtbar** — beide Fassungen sahen wie gültige Kurven aus, die korrigierten Werte fielen deutlich *zugunsten* des Verfahrens aus (λ=0,02: 3,410 → 8,210). Aufgefallen ist er, weil die Kontrolle unverändert blieb. Das gehört als Fallbeispiel in Kapitel 9 oder 7, nicht als Fußnote |
| Q41 | **Vier neue Runbooks sind PowerShell-only, zwei tragen fest eingetragene Lauf-Verzeichnisse.** `run_lambda_sweep.ps1`, `run_relevance_queue.ps1`, `rerun_lambda_arms.ps1` und `eval_training_curve.ps1` setzen Windows voraus; `build_training_curve.py` und `eval_training_curve.ps1` listen die Laufpfade hart, weil die Experimentnamen Präfixe voneinander sind (`…lambda0` steckt in `…lambda01`) und eine Teilstringsuche bereits einmal den falschen Lauf getroffen hatte — die Kontrolle wurde dadurch gegen den λ=0,1-Checkpoint ausgewertet | 03 / 00 | 05, 09 | ○ | **Neu angelegt 2026-08-21** · Verwandt mit Q18 (Windows-spezifische Umgebungsvariable) und Q15 (Launch-Shim). **Die Präfixkollision ist die eigentliche Pointe:** Sie fiel nur auf, weil zwei Arme *exakt identische* Werte lieferten. Als Reproduzierbarkeitsgrenze zu nennen, nicht als Werkzeug |
| Q42 | **Ein Logging-Defekt hat 64 Aufrufstellen still betroffen.** `RankedLogger.log` hatte `rank` zwischen `msg` und `*args`; der Standardaufruf `log.info("loaded %s", path)` band damit `path` an `rank`. Folge: Platzhalter wurden wörtlich ausgegeben oder warfen `TypeError`, **und** die Meldung wurde ganz verworfen, sobald der so entstandene „Rang" nicht dem aktuellen entsprach. Seit 2026-08-17 ist `rank` keyword-only | 03 | 07, 09 | ○ | **Neu angelegt 2026-08-21** · **Die belegrelevante Lehre ist nicht der Bug, sondern seine Form:** Eine fehlende Logausgabe bedeutet nicht, dass der Codepfad nicht lief. Wer Laufprotokolle aus der Zeit vor dem 2026-08-17 als Nachweis heranzieht, muss das wissen. Umfang: ein Halbsatz, ggf. in derselben Aufzählung wie Q10 |
...
| **~~29~~ 33 Experimentkonfigurationen existieren, ~8 Läufe sind im Vault dokumentiert.** | [10 §Beobachtungen](10_konfiguration.md) | Im Beleg zwischen *implementiert* und *durchgeführt* trennen. **Präzisierung 2026-08-21:** Die sechs Relevanz-Experimente **sind gelaufen und ausgewertet** — nur eben nicht im Vault, sondern unter `docs/results/` und in `docs/relevance_regularization.md` §13. |
| **Die Ergebnisdokumentation hat den Ort gewechselt.** Bis Juli 2026 lagen die Ergebniszahlen im Vault; die xAI-Lokalisierungsstrecke vom August 2026 dokumentiert ihre Ergebnisse ausschließlich in `docs/`. Zu keinem der sechs Relevanz-Läufe existiert eine Vault-Notiz. | [12](12_dokumentation_vault.md) | **Wer für `06Results.tex` nur den Vault abgleicht, übersieht F25d, F60 und F63 vollständig** — also das Hauptergebnis der xAI-Strecke. |
...
| **V5** | **Der Phase-3-Sweep mischt zwei Datenstufen:** die evaluierten Checkpoints wurden auf der 32-Identitäten-Stufe trainiert, die Evaluation läuft auf 1471 Testvideos der 165-Identitäten-Stufe. Deshalb liegt die dort berichtete Clean-AUC (0,857) unter der Baseline-AUC (0,999). | [12 §3.2](12_dokumentation_vault.md), Zeile G10 | Die Asymmetrie gehört **benannt** in `05Experimental_Setup.tex`; die Leckagefreiheit ist über den deterministischen Hash begründet. | offen · **in `00Abstract.tex` Absatz 4 als Vorbehaltssatz vorweggenommen (2026-08-07)**, weil dort 0,999 und 0,857 zwei Absätze auseinanderstehen; die ausführliche Begründung bleibt Aufgabe von Kapitel 5 |
...
| **P0** | **Das Lokalisierungsergebnis selbst** | F25d–F25g | **Neu 2026-08-21 und die größte offene Lücke der Matrix.** Fünf Messpunkte mit Konfidenzintervallen, ein Kontrollarm, der die naheliegende Alternativerklärung ausschließt, und ein Anti-Gaming-Nachweis — nichts davon steht im Beleg, auch nicht in einer Skizze. `06Results.tex` hat 14 KB. |
| **P0** | Lokalisierungsmetriken als Metrikkapitel | E13 | Die **erste quantitative xAI-Metrik des Projekts**. Ohne ihre Definition (insbesondere die Chance-Normierung) ist jede Zahl aus F25d unlesbar. |
| **P0** | Chefer-Ablation als Gegenprobe | F60, F61, F65, F66, F57 | Beantwortet den Zirkelschluss-Einwand gegen F25d, seit dem 2026-08-22 auf 624 Clips statt 17. **Zugleich der Grund, warum F57 erneut `!` ist** — der Beleg sagt inzwischen an vier Stellen, Rollout sei nicht implementiert. **F65 gehört zwingend dazu:** die Gegenprobe bestätigt die *Richtung* uneingeschränkt und die *Höhe* nur beim Pointing Game. |
| **P0** | Masken als zweiter Ground-Truth-Bestand | A48–A51 | Ein eigener Datenbestand mit eigenem Gate (G0). Ohne ihn ist F25d nicht nachvollziehbar; die Variantenasymmetrie (A51) betrifft zudem die kategorienweise Auswertung. |
| **P1** | Aux-Head als Negativbefund | F63 | **Inhaltlich vielleicht der interessanteste Punkt des ganzen Blocks** und am leichtesten zu übersehen, weil er wie ein gescheitertes Nebenexperiment aussieht. |
| **P1** | Manuelle Optimierung und das 8-GB-Budget | D50, D51 | Setup-Kapitel. Der Befund „AttnLRP ist billiger als Input×Gradient" ist kontraintuitiv und berichtenswert. |
| **P1** | Der lxt-Patch verändert das Backward | F58, F59, F62 | Drei zusammenhängende stille Fehlerklassen, alle geschlossen und gemessen — dieselbe Erzählform wie F12, die im Beleg bereits funktioniert. |
| **P2** | Frame-/Masken-Ausrichtung | B22–B24 | Klingt nach Datenladelogik, ist aber die Bedingung dafür, dass überhaupt das richtige Ziel optimiert wird. |
| **P2** | Verlorene Läufe durch Checkpoint-Politik | Q40 | Fallbeispiel für Kapitel 7 oder 9: Der Fehler war im Ergebnis unsichtbar und fiel nur auf, weil die Kontrolle sich *nicht* änderte. |
